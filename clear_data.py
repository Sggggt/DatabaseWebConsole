"""
Utility script to purge data from the database defined in db_config.ini.
...
def clear_database(drop: bool, force: bool) -> None:
    ...
    if not settings["database"]:
        raise SystemExit("Set a database name in db_config.ini before running this tool.")
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Purge data from the configured database.",
    )
    parser.add_argument(
        "--drop",
        action="store_true",
